...
- **아래 두 가지는 여기에 해당하지 않습니다. 인터페이스 탓으로 돌리지 마세요.**
  - `Timed out while waiting for action server to acknowledge goal request for ...` → `bt_navigator` 의 `default_server_timeout` 초과입니다 (위 Nav2 구성 참고).
...
| 파라미터 | 값 | 무엇 |
|---|---|---|
| `staging_distance` | 1.05 m | **Nav2 가 데려다 주는 곳.** 여기까지만 Nav2, 이후는 마커 |
| `approach_distance` | 0.65 m | 정렬 지점. 여기서 측정·보정을 반복 |
| `rotate_distance` | 0.60 m | 180도 회전 지점 |
| `dock_distance` | 0.254 m | 도킹 완료 자세 |
| `rear_target` | 0.46 m | 회전 후 뒤쪽 라이다가 읽어야 할 거리 |

- **Nav2 목표와 정렬 지점을 분리해야 합니다.** 정렬 지점(0.65)을 Nav2 목표로 쓰면 코스트맵 팽창(0.40 m)이 칠한 벽 앞 영역이라 도크에 바짝 붙어 멈춥니다.
- **`dock_distance` 를 바꾸면 `rear_target`, `navigation.launch.py` 의 스폰 `y`, `docking.yaml` 의 `dock_y` 가 함께 움직입니다.** 마커면 위치는 월드에 고정이라 도킹 자세가 `마커면 + dock_distance` 로 결정됩니다.

**마커 — ArUco `DICT_4X4_50`, 0.10 m 3장(id 1/0/2)을 좌우 0.16 m 간격, 높이 0.31 m**
...
- **마커 높이를 낮추지 마세요.** 카메라 화각 때문에 h = 0.316 에서 검출 시작 거리가 최소이고, h = 0.20 이면 0.30 m 에서 이미 화면 아래로 잘립니다. 유도는 `generate_room.py` 참고.
- **검출 절벽은 로봇 중심 0.516 m 이고, 세로가 아니라 가로입니다** — 바깥 마커가 화각을 벗어납니다. 마커를 내려도 이 절벽은 움직이지 않습니다.
...
- **거리 편차는 거리에 비례해 커집니다** (마커가 작게 보일수록 코너가 안쪽으로 잡힘). 보정치는 **가장 가까운 검출 지점의 편차**로 잡으세요.
...
- **회전 뒤 후진은 뒤쪽 라이다로 닫습니다** (`use_rear_lidar`). 회전하면 마커가 등 뒤라 후진이 개루프가 되고 회전 중 생긴 밀림이 그대로 세로 오차로 남습니다. 라이다는 360도라 뒤쪽 빔이 이미 있고, 스캔 평면이 도크 패널보다 높아 그 너머 벽을 직접 봅니다. 각 빔을 후진 축으로 투영(`r·cos`)해 비스듬한 빔의 편향을 없애고 중앙값을 씁니다. **이것으로 세로 오차가 49.9 → 4.9 mm 로 줄었습니다.**
- **회전점을 정하는 것은 섀시가 아니라 캐스터입니다.** 캐스터 구 반지름이 0.030 m 뿐이라 높이 0.040 m 인 동판 턱을 못 넘고 걸립니다. **늘리는 쪽은 안전하고, 줄일 때만 이 여유를 다시 확인해야 합니다.**
- 회전점은 검출 절벽 위여야 회전 직전에 마커로 다시 잴 수 있습니다. 후진량은 고정값이 아니라 그때 잰 거리에서 `dock_distance` 를 뺀 값이라 **회전점 오차가 최종 자세로 전달되지 않습니다.**
- 회전 구간에는 `vodom_tf_relay` 의 `~/pause` 로 시각 오도메트리 보정을 얼립니다. **얼린 동안에도 TF 는 계속 나가야 합니다** — 끊으면 `map` 과 `base_footprint` 가 다른 트리로 갈라집니다.
- 월드 생성기의 `REVERSE_DOCK` 과 `docking.yaml` 의 `reverse_dock` 은 **함께** 바꿔야 합니다.

**단계 분리 정렬 (`staged`)**

정지 → 측정 → 보정 → 정지 → 재측정 을 반복합니다. 차동구동은 옆으로 못 가므로 횡오차는 **크랩 기동**(회전 30도 → 직진 → 회전 −30도)으로 없앱니다.

- **크랩의 직진은 후진 방향입니다.** 전진으로 하면 도크 쪽으로 횡오차의 1.73배만큼 다가가 보정할수록 여유가 줄어듭니다.
- **크랩 부호를 뒤집으면 오차가 두 배가 되고**(76 → 154 mm) 그 뒤 검출까지 잃습니다. 오차가 커지면 즉시 중단하는 감시가 들어 있습니다.
- **정렬 판정은 축별 허용치가 아니라 "접촉 시점 예상 횡오차" 하나로 합니다.** 정렬 후 직진하는 동안 각도는 그대로 횡오차가 됩니다:
  ...
  축별로 재면 서로 모순된 목표가 되어 루프가 수렴하지 않습니다. `contact_lateral_budget: 0.006` 하나로 판정하고, **크랩 보정량도 측정값이 아니라 이 예상값을 씁니다.** `yaw_tolerance` 는 정밀도용이 아니라 측정 이상 감시용입니다.
- 크랩 기동의 분해능이 바닥이라 **가로는 3~5 mm 아래로 못 내려갑니다.** 허용치 ±34 mm 대비 무시할 수준입니다.
- **진입점 도착은 액션 상태가 아니라 좌표로도 확인합니다.** 목표가 선점되면 핸들은 ABORTED 를 받지만 로봇은 실제로 도착해 있습니다.

**실측 (room.sdf, `staged` + 후진, 연속 3회)**: 세로 +3.0 / +6.2 / +7.0 mm, 가로 +1.1 / −0.3 / +1.4 mm. 허용치는 세로 ±48 mm / 가로 ±34 mm.
...
- 브래킷은 **0.030 ~ 0.060 m 사이**에만 놓을 수 있습니다. 아래로 내리면 코스트맵이 안 보는 낮은 물체에 걸리고, 위로 올리면 섀시 안에 들어가 동판에 닿지 않습니다.
- 좌우로는 바퀴가 지나가지 않는 띠가 **|y| < 0.11** 뿐입니다. 동판이 바퀴 경로에 걸리면 로봇이 그 위로 올라타 간격이 유지됩니다.
- **접촉 허용치는 브래킷이 아니라 핀 배열(6.1 × 3.5 mm)이 동판(75 × 100 mm) 위에 얹히는 범위입니다** — 세로 ±48 mm / 가로 ±34 mm. 브래킷 기준으로 잡으면 실제로는 닿아 있는데 "충전 안 됨"이 되어 있지도 않은 정확도를 요구하게 됩니다.
- **가이드 벽은 두지 않습니다.** 실물 스테이션에 없고, 내접 반경 2배보다 좁은 통로는 안쪽 전체가 `INSCRIBED_INFLATED_OBSTACLE` 로 칠해져 **코스트맵 충돌 검사를 원리적으로 쓸 수 없습니다.**
- **실기 확인 필요**: 핀이 브래킷 아래로 5 mm 이상 나와야 하는데 2.54 mm 피치 포고핀 스트로크가 보통 1.5~2.5 mm 라, 자유 길이로 5 mm 를 메우고 **남는 만큼만 압축 여유**가 됩니다. 그것이 곧 바닥 높이 편차 허용치입니다.

**도킹 설정에서 반드시 지킬 것**

- **`docking_server` 의 `enable_stamped_cmd_vel: true`.** 기본값 false 로 두면 `Twist` 를 내보내고 `diff_drive_controller` 가 조용히 무시합니다. 증상은 "도킹은 진행되는데 로봇이 제자리".
- **`use_collision_detection: false`.** 도크 앞 0.7 m 는 일부러 벽에 붙으러 가는 구간이라 충돌 검사의 전제가 성립하지 않습니다. `dock_collision_threshold` 로 예외 처리하는 것은 동작하지 않습니다. **실기에서는 그 구간을 위해 범퍼 스위치나 모터 전류 제한 같은 소프트웨어 밖의 보호가 필요합니다.**
- **`use_battery_status: false`.** 접근 루프가 `isCharging()` 으로도 빠져나오는데 접점 허용치가 넓어 목표보다 한참 앞에서 끝납니다. 역할을 나눕니다 — 접근은 `docking_server`, 충전 확인은 `auto_dock.py` 가 `/battery_state` 로.
- **접촉 판정에 SLAM 자세를 쓰지 마세요.** 접촉은 물리적 사실이므로 `battery_sim.py` 는 Gazebo 지상 진실로 판정합니다. 그 허용치는 `docking_threshold` 보다 **커야** 합니다.
- **`/world/<world>/dynamic_pose/info` 를 `tf2_msgs/TFMessage` 로 브리지하지 마세요.** 프레임 이름을 담는 `header.data` 가 없어 `frame_id` 가 전부 빈 문자열로 나옵니다. URDF 에 `gz-sim-odometry-publisher-system` 을 붙여 `nav_msgs/Odometry` 로 받으세요.
- **자세를 고정할 때 영상 타임스탬프로 TF 를 조회하지 마세요.** 영상이 `odom` TF 보다 먼저 도착해 "extrapolation into the future" 로 매번 실패합니다. **`timeout` 을 줘도 소용없습니다** — 그 콜백이 실행기를 잡고 있어 TF 콜백이 그 사이 돌 수 없습니다. 가장 최근 TF(stamp=0)로 조회하세요.

**도크 좌표 자동 등록** (`scripts/dock_register.py`, `dock_register:=true`)

도크에 붙은 채 부팅하면 그 자세를 도크 좌표로 등록합니다. 스테이션을 옮겨도 로봇을 한 번 밀어 넣고 재부팅하면 됩니다. Nav2 `dock_database` 규격 파일(`~/.ros/orinbot_docks.yaml`)로 쓰고, 도는 `staged_dock` 에는 `SetParameters` 로 바로 반영합니다. 부팅 때가 아니어도 `~/register` 로 부를 수 있습니다.

- **저장하는 것은 "도크 앞"이 아니라 "도킹 완료 자세"입니다.** 진입점은 `staged_dock` 이 계산하므로, 앞으로 빼서 저장하면 두 번 빠집니다.
- **후진 도킹이면 yaw 에서 180도를 뺍니다.** `dock_yaw` 는 접근 방향인데 최종 자세는 정반대입니다. 부호를 틀리면 증상이 "마커 검출 실패"로만 보입니다. `reverse_dock` 은 `staged_dock` 쪽과 **같아야** 합니다.
- **등록 좌표는 `map` 기준이고 그 원점은 로봇이 부팅한 자리입니다.** 도크에서 부팅하면 `(0, 0)` 이 나오는 것이 정상입니다.
- **도킹 여부는 충전 전류로 판정하되 첫 샘플로 판단하면 안 됩니다** — 기동 직후에는 접촉 판정 입력이 아직 없어 방전으로 나옵니다 (`charge_wait`).
- **등록 대기는 실행기를 블록하므로 `MultiThreadedExecutor` + `ReentrantCallbackGroup` 이어야 합니다.** 단일 스레드로 두면 안정화 판정이 갱신 없는 버퍼에서 같은 값만 읽어 무조건 통과하고, 파라미터 반영은 성공했는데도 실패로 보고됩니다.
- **`scripts/*.py` 에 실행 권한이 없으면** 런치가 `executable not found` 로 **스택 전체를 중단**시킵니다. 새 스크립트에는 `chmod +x` 를 확인하세요.
...
- **`.py` 의 주석과 독스트링은 영어로, 짧게 씁니다.** 한 줄로 끝내고, 왜 그런지가 꼭 필요할 때만 한 줄 더 붙입니다. 수치의 유도 과정·실측표·대안 검토는 주석이 아니라 `CLAUDE.md` 나 `docs/ros2-lessons.md` 로 보냅니다. 로그 메시지와 서비스 응답은 한국어 그대로 둡니다 — 시험 중에 읽는 출력입니다.
