"""고정 시나리오를 돌리면서 노드별 CPU/메모리를 샘플링한다.

파라미터를 바꿀 때마다 이걸 돌려서 "유의미한 차이"인지 판단한다.
같은 주행을 시키므로 실행 간 비교가 가능하다.

  python3 measure_load.py <라벨>

시나리오: 0.60 m 통로를 지나 (4.3,-3.0) 까지 갔다가 (0,0) 으로 복귀.
RViz 와 Gazebo 는 실기에 없으므로 합계에서 제외하고 따로 표시한다.

CPU 측정 방식 (2026-08-02 수정)
-------------------------------
예전에는 `ps -eo pcpu` 를 썼는데, 이 값은 **프로세스가 뜬 뒤 지금까지의
평균**이다. 노드는 시나리오 시작 몇 분 전에 떠서 대부분 놀고 있었으므로
주행 구간의 실제 부하가 희석돼 낮게 나왔다. 파라미터 A/B 비교에서는
양쪽이 같은 방향으로 눌리므로 상대 비교는 유효했지만, "Orin 6코어 중
몇 코어" 같은 절대값은 과소평가였다.

지금은 /proc/<pid>/stat 의 utime+stime 을 연속 샘플의 차분으로 읽는다.
이것이 그 0.5초 구간의 진짜 사용률이다.
"""
import os
import subprocess
import sys
