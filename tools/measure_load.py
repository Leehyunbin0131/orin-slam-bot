...
시나리오: c070 (폭 0.70 m) 통로를 지나 (4.3,0.6) 까지 갔다가 (0,0) 으로 복귀.
RViz 와 Gazebo 는 실기에 없으므로 합계에서 제외하고 따로 표시한다.

**`ps -eo pcpu` 를 쓰면 안 된다.** 그 값은 프로세스가 뜬 뒤의 누적 평균이라,
시나리오 몇 분 전에 떠서 놀고 있던 노드의 주행 구간 부하가 희석된다
(A/B 상대 비교는 유효하지만 절대값은 과소평가). 지금은 /proc/<pid>/stat 의
utime+stime 을 연속 샘플의 차분으로 읽는다 — 그 0.5초 구간의 진짜 사용률이다.
"""
import os
import subprocess
import sys
