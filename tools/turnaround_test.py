...
왜 필요한가
-----------
통로 시험이 "직진해서 빠져나가는가"만 보면 부족하다. 로봇은 막다른 곳에서
되돌아 나와야 하고, 차동구동은 후진보다 제자리 회전을 먼저 시도한다
(Nav2 의 spin 복구도 마찬가지다).

  로봇 대각선 = sqrt(2) * 0.40 = 0.5657 m  <- 제자리 회전 최소 폭
  편측 여유: 0.60 m 통로 -> 17 mm,  0.70 m 통로 -> 67 mm

17 mm 는 실측 SLAM 자세 오차(중앙값 21 mm)보다도 작다.

시험 방법
---------
Nav2 를 거치지 않는다. 로봇을 통로 한가운데 놓고 /cmd_vel_teleop 으로 제자리
회전만 시킨 뒤, Gazebo 실제 자세로 360도를 다 돌았는지와 도는 동안 중심이
얼마나 밀렸는지를 잰다. sim.launch.py 만 있으면 되고, **SLAM 은 띄우지 말 것**
— 순간이동 때문에 지도가 망가진다.
"""
import math
import subprocess
