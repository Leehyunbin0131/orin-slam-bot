...
허용치는 `generate_room.py` 의 도크 치수에서 옵니다 — 핀 배열이 동판
(75 x 100 mm) 위에 얹히는 범위입니다. 빼는 것은 브래킷(30 mm 각)이 아니라
**핀 배열 6.1 x 3.5 mm** 입니다 (브래킷은 동판 상면보다 5 mm 위입니다).
  - 세로 ±48 mm, 가로 ±34 mm  = (동판 - 핀 배열) / 2
  - 각도 ±5도 — 가이드 벽이 없어 순전히 인식·제어 품질입니다. 각도가 핀을
    직접 밀어내지는 않지만 **가로 여유를 잡아먹습니다** (커넥터가 좌우
    0.055 m 라 5도면 4.8 mm).
"""
