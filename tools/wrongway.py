#!/usr/bin/env python3
"""로봇이 **가야 할 방향과 반대로 도는 순간**을 잡아 기록한다.

    python3 tools/wrongway.py [측정초]

증상 정의
---------
"오른쪽으로 빠져나가야 하는데 갑자기 왼쪽으로 꺾는다."
이를 다음으로 정의합니다:

    경로_방향오차 = (경로를 따라 LOOK m 앞 지점의 방향) - (로봇 헤딩)
    이 오차가 THRESH 이상인데, 각속도 명령의 부호가 오차와 **반대**

즉 "왼쪽으로 가라는데 오른쪽으로 돌린다(또는 그 반대)"인 순간입니다.
발생한 좌표 · 오차 크기 · 지속 시간을 모아 보고합니다.

같이 기록하는 것
----------------
- 경로가 로봇 근처에서 **접혀 되돌아오는지** (U자 통로에서 반대편 다리가
  똑같이 가까우면, 거리만 보는 PathAlignCritic 이 엉뚱한 쪽에 붙습니다)
- 그 순간의 전진 속도 명령

전제: 시뮬레이터와 Nav2 가 실행 중이고 목표가 진행 중이어야 합니다.
"""
