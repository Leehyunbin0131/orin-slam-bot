...
# 후진 도킹입니다 — 정렬은 도크를 마주 본 채로 하고, 회전점에서 180도 돌아
# 뒤로 들어갑니다. 충전 내내 카메라가 벽이 아니라 방을 보게 하려는 것입니다
# (벽 0.3 m 앞에서는 시각 오도메트리가 깨져 지도가 망가집니다).
#
#   패널 앞면 -3.90 / 마커면 -3.894 / 로봇 외접반경 0.283 / 포고핀 로봇 뒤 0.10
#   회전점 y=-3.594 (마커면 0.30 m) — 패널까지 0.306 이라 제자리 회전 가능
#   도킹   y=-3.67  (마커면 0.224 m) — 섀시 뒷면이 패널에서 0.03 m
#   후진 거리 0.076 m
