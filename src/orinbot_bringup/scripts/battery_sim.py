...
충전 여부 판정에 대하여 (실측으로 두 번 고친 부분입니다)
--------------------------------------------------------
실기는 접점에 전기가 흐르는지로 압니다. 시뮬레이터에는 전기가 없으므로
"동판 앞에 제대로 서 있는가"를 기하로 대신 판정합니다.

**SLAM 자세(map->base_footprint)를 쓰면 안 됩니다.** 접촉은 물리적
사실인데 SLAM 자세에는 중앙값 21 mm / 90% 46 mm 의 오차가 있습니다.
처음에 SLAM 자세 + 여유 0.15 m 로 두었더니, 로봇이 아직 100 mm 앞에
있는데 "충전 시작"으로 판정했고 `docking_server` 의 접근 루프가
(isDocked 뿐 아니라 isCharging 으로도 빠져나오기 때문에) 거기서
끝나 버렸습니다. 실측 세로 오차 중앙값이 119 mm 였습니다.
그래서 Gazebo 지상 진실(`/ground_truth/odom`)을 씁니다.

허용치는 도크 기하에서 나옵니다 — 포고핀 블록(30 mm 각)이 동판 위에
완전히 얹혀 있는 범위입니다 (generate_room.py 가 계산해 찍어 줍니다).
빼는 것은 브래킷(30 mm 각)이 아니라 **핀 배열**입니다. 커넥터가 6핀
2x3 / 피치 2.54 mm 라 실제로 닿아야 하는 면적은 6.1 x 3.5 mm 뿐이고,
브래킷은 커넥터를 붙드는 구조물이라 동판 밖으로 걸쳐도 무방합니다.
브래킷 기준으로 판정하면 실제로는 닿아 있는데 "충전 안 됨"이 됩니다.
  세로 : ±48 mm  (동판 길이 100 mm - 핀배열 3.5 mm) / 2
  가로 : ±34 mm  (동판 폭 75 mm - 핀배열 6.1 mm) / 2
  각도 : ±6.3도 — 가이드 벽이 없으므로 물리적으로 막아 주는 것은
         없습니다. 세 축을 따로 보는 근사이고, 각도가 크면 가로
         여유를 잡아먹습니다 (커넥터가 좌우 0.055 m 에 있어 6.3도면
         6 mm 가 그쪽으로 갑니다).

전류 모델
---------
Jetson Orin Nano Super + 모터 2개 + 센서 기준의 대략치입니다.
정확한 값이 목적이 아니라 "움직이면 더 닳는다" 는 성질이 목적입니다.
"""
