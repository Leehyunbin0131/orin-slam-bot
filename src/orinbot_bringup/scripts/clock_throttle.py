...
왜 필요한가 (실측)
------------------
`use_sim_time: true` 인 **rclpy** 노드는 /clock 메시지마다 파이썬
콜백을 돌며 시계를 갱신합니다. 그 비용이 메시지당 고정이라 발행
주파수에 그대로 비례합니다.

  개발 PC (i7-14700K) : 997 Hz 에서 노드당 24.7%   (vs use_sim_time=false 0~2.3%)
  Jetson Orin Nano    : 100 Hz 에서 노드당 11.3%p  (vs 0.2%)

Orin 은 코어당 성능이 1/4~1/5 이라 997 Hz 면 **노드당 약 113%p**,
즉 rclpy 노드 6개로 6코어를 통째로 씁니다. 실기에는 /clock 자체가
없으므로 이 부하는 순수한 시뮬레이션 인공물인데, 그것 때문에
시나리오가 못 돌아가는 상황이 됩니다.

물리 스텝(`max_step_size` 1 ms)은 건드리지 않습니다. 시뮬레이션의
정확도는 그대로 두고 **ROS 로 나가는 시계의 해상도만** 낮춥니다.
100 Hz 면 시각 분해능이 10 ms 인데, 이 시스템에서 가장 빠른 주기가
컨트롤러 20 Hz(50 ms)이고 TF/센서 동기화는 노드 시계가 아니라 메시지
헤더 스탬프를 쓰므로 영향이 없습니다.

`clock_rate:=0` 으로 두면 솎지 않고 그대로 통과시킵니다.
"""
