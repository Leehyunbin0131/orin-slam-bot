#!/usr/bin/env python3
"""단계를 끊어서 도킹한다 (`opennav_docking` 대체).

    /dock_robot   (nav2_msgs/DockRobot)
    /undock_robot (nav2_msgs/UndockRobot)

액션 규격은 `opennav_docking` 과 같습니다. 그래서 `auto_dock.py` 나
BT 노드는 어느 쪽을 쓰든 고칠 것이 없습니다.
`docking.launch.py` 의 `docking_mode` 인자로 고릅니다.

왜 단계를 끊는가 (실측 근거)
----------------------------
정지 상태에서 마커 3장 보드로 재면 각도 정확도가 **0.01도**입니다.
그런데 매끄러운 접근(opennav_docking 의 smooth control law)으로 붙이면
최종 각도 오차가 **1.8도** 나옵니다. 차이는 인식이 아니라 제어에서
생깁니다 — Nav2 의 staging 도착 허용 오차가 ±200 mm / ±14.3도인데,
그 오차를 0.65 m 안에 다 펴야 하니 다 못 펴고 도착합니다.

그래서 **정지 → 측정 → 보정 → 정지 → 재측정** 을 반복해 정지 상태의
정확도를 그대로 최종 자세로 가져가고, 마지막은 순수 직진만 합니다.

횡방향 오차는 왜 따로 다루는가
------------------------------
차동구동은 옆으로 못 갑니다. 그래서 횡오차 e 를 없애려면
**회전 t -> 직진 e/sin(t) -> 회전 -t** (크랩 기동)를 해야 합니다.
t=30도면 e=10 mm 에 직진 20 mm 라 회전 슬립이 거의 없습니다.

무엇을 "정렬 완료"로 볼 것인가 — 축별로 재지 않습니다
------------------------------------------------------
처음에는 횡오차와 각도오차에 각각 허용치를 두었습니다(3 mm / 0.5도).
그런데 이 둘은 **독립이 아닙니다.** 정렬을 마치고 직진하는 동안
각도오차 psi 는 그대로 횡오차로 바뀝니다:

    접촉 시점 횡오차 = 정렬시 횡오차 - 직진거리 * sin(psi)

0.52 m 직진 기준으로 각도 0.5도가 4.5 mm, 1도가 9.1 mm 입니다.
즉 **각도 허용치 0.5도만으로도 횡 허용치 3 mm 보다 큰 오차가
생깁니다.** 3 mm 를 맞추려고 크랩을 반복하는 동안 그보다 큰 오차를
각도 쪽에서 허용하고 있었던 셈이라, 두 허용치가 서로 안 맞았습니다.
(실측: 횡 -3.1 -> +3.1 -> +0.4 mm 로 헛도는 반복이 회당 15~20초.)

그래서 축별 허용치를 버리고 **물리적으로 의미 있는 양 하나**로
판정합니다 — 접촉 시점의 예상 횡오차. 동판 허용치가 ±34 mm 이므로
그 절반인 15 mm 를 예산으로 잡습니다. 각도가 작으면 횡을 더 봐주고,
횡이 작으면 각도를 더 봐주는 식으로 서로 상쇄됩니다.

각도에는 별도로 느슨한 상한(1도)만 둡니다. 이건 정밀도 때문이 아니라
"측정이 이상하지 않은지" 확인용입니다 — 1도면 위 식으로 9 mm 라
예산 안에서 크랩이 흡수할 수 있고, 접촉 자세 자체도 포고핀 브래킷
좌우 간격(±55 mm)에 1도를 곱해 1 mm 밖에 안 틀어집니다.

크랩은 lat 이 아니라 **예상 접촉 오차** 만큼 옮깁니다. 그래야
고치지 않고 남겨 둔 각도가 만들 흘러감까지 같이 상쇄됩니다.

주의: 회전은 마커가 아니라 **오도메트리**로 닫습니다. 마커는 정지
상태에서만 읽습니다 (움직이며 읽으면 그 값이 왜 흔들리는지 알 수 없게
됩니다). 짧은 회전 구간의 휠 오도메트리는 mm/0.1도 수준입니다.
"""
