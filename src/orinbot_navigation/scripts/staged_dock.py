...
왜 단계를 끊는가
----------------
정지 상태에서 마커 3장 보드로 재면 각도 정확도가 0.01도인데, 곡선 접근으로
붙이면 최종 각도 오차가 1.8도입니다. 차이는 인식이 아니라 제어에서 생깁니다
— staging 도착 오차(±200 mm / ±14.3도)를 0.65 m 안에 다 펴지 못합니다.
그래서 **정지 -> 측정 -> 보정 -> 정지 -> 재측정** 으로 정지 상태의 정확도를
그대로 최종 자세로 가져가고, 마지막은 순수 직진만 합니다.

차동구동은 옆으로 못 가므로 횡오차 e 는 크랩 기동으로 없앱니다:
**회전 t -> 직진 e/sin(t) -> 회전 -t**. t=30도면 e=10 mm 에 직진 20 mm 라
회전 슬립이 거의 없습니다.

정렬 완료 판정 — 축별로 재지 않습니다
--------------------------------------
횡오차와 각도오차는 **독립이 아닙니다.** 정렬 후 직진하는 동안 각도가
그대로 횡오차로 바뀝니다:

    접촉 시점 횡오차 = 정렬시 횡오차 - 직진거리 * sin(psi)

0.52 m 직진에서 0.5도 = 4.5 mm, 1도 = 9.1 mm. 그래서 옛 설정(횡 3 mm /
각도 0.5도)은 서로 안 맞았습니다 — 허용한 각도만으로 횡 허용치보다 큰
오차가 생기는데 3 mm 를 맞추려고 크랩을 반복했습니다(회당 15~20초 손해).

지금은 **접촉 시점의 예상 횡오차 하나**로 판정하고(동판 허용 ±34 mm 의
절반인 15 mm), 크랩도 lat 이 아니라 이 예상값만큼 옮깁니다 — 그래야 남겨
둔 각도가 만들 흘러감까지 함께 상쇄됩니다. 각도의 느슨한 상한(1도)은
정밀도용이 아니라 측정 이상 감시용입니다.

주의: 회전은 마커가 아니라 **오도메트리**로 닫고, 마커는 정지 상태에서만
읽습니다. 짧은 회전 구간의 휠 오도메트리는 mm/0.1도 수준입니다.
"""
