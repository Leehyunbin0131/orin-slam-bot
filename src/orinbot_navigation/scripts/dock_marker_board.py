...
------------------------------------------------
그 노드는 마커 **한 장**만 추적합니다. 정면에 가까운 평면 마커 하나는
자세 모호성(planar pose ambiguity) 때문에 각도가 크게 튑니다 — 실측에서
1.3 m 거리에 4.77도까지 흔들렸습니다. 가이드 벽이 있으면 그 오차를 벽이
기계적으로 잡아 주지만, **이 스테이션에는 가이드 벽이 없어 최종 각도를
전적으로 인식이 결정합니다.**

마커 3장을 좌우로 벌려 놓고 코너 12개를 함께 풀면(estimatePoseBoard)
yaw 가 강하게 구속됩니다. 좌우로 벌린 배치가 핵심입니다 — 세로로 쌓으면
yaw 개선 효과가 거의 없습니다.

"60 cm 에서 확정하고 직진"
--------------------------
가까이 갈수록 마커는 커지지만 화각을 벗어나기 시작하고, 마지막 몇
프레임이 가장 부정확합니다. 그런데 docking_server 는 마지막 검출값을
그대로 목표로 얼려서 들어가므로, **가장 나쁜 관측으로 마무리하게**
됩니다 (실측 세로 오차 -25 ~ +46 mm).

그래서 `lock_distance` 안으로 들어오면 그 시점의 자세를 고정 프레임
(odom)으로 한 번 변환해 두고, 이후로는 그 값을 시각만 갱신하며 계속
내보냅니다. 로봇 입장에서는 "확정된 목표를 향해 그냥 직진"이 됩니다.

  주의: 고정할 때 반드시 odom 프레임으로 바꿔야 합니다. 카메라 광학
  프레임 그대로 얼려서 다시 내보내면 목표가 로봇과 함께 움직여
  영원히 도달하지 못합니다.
"""
